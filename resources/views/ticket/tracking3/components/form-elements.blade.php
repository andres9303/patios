    ...
    @foreach($ticket->trackings as $tracking)
        <div class="flex items-center mb-4 p-4 rounded-lg shadow
            ...
            <div class="w-1/4 text-gray-700 text-right">
                ...
                <a href="{{ route('resolve-3ticket.attachment.index', ['ticket' => $ticket->id, 'resolve_3ticket' => $tracking->id]) }}" 
                    class="bottom-2 text-xl right-2 text-yellow-500 hover:text-yellow-700">
                    <i class="fas fa-paperclip"></i>
                </a><br>
                <a href="{{ route('resolve-3ticket.edit', ['ticket' => $ticket->id, 'resolve_3ticket' => $tracking->id]) }}" 
...
<div class="text-gray-500">
    ...
    <div class="mt-4">
        ...
        {!! html()->date('date', $resolve_3ticket->date ?? now()->format('Y-m-d'))->class('block mt-1 w-full' . ($errors->has('date') ? ' is-invalid' : '')) !!}
    </div>
    <div class="mt-4">
        {!! html()->label('Detalle')->for('text') !!}
        {!! html()->textarea('text', $resolve_3ticket->text ?? null)->class('block mt-1 w-full' . ($errors->has('text') ? ' is-invalid' : ''))->placeholder('Detalle del seguimiento') !!}
    </div>
    <div class="mt-4">
        {!! html()->label('Tipo')->for('type') !!}
        {!! html()->select('type', ['0' => 'Seguimiento', '1' => 'Solución', '2' => 'Feedback', '3' => 'Cancelar Ticket'], $resolve_3ticket->type ?? 0)->class('block mt-1 w-full' . ($errors->has('type') ? ' is-invalid' : '')) !!}
    </div>
</div>