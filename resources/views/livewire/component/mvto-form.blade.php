            ...
            <div class="mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 bg-white rounded-xl shadow-2xl p-6 space-y-6">
                    ...
                    {!! Html::form('POST', route($route))->class('space-y-4')->open() !!}
                        ...
                        <div>
                            {!! Html::label($labelCode, 'code')->class('block text-sm font-medium text-gray-700') !!}
                            {!! Html::text('code', $code ?? '')
                                ...
                                ->attribute('placeholder', 'Ingrese '.$labelCode) !!}
                            @error('code') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            {!! Html::label($labelDocument, 'num')->class('block text-sm font-medium text-gray-700') !!}
                            {!! Html::text('num', $num ?? '')
                                ->class('mt-1 block w-full bg-white text-gray-900 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500')
                                ->attribute('placeholder', 'Ingrese '.$labelDocument) !!}
                                ...
                                <table class="min-w-full bg-white divide-y divide-gray-300">
                                    <thead class="bg-indigo-600">
                                        <tr>
                                            ...
                                            @if($isActivities)<th class="px-4 py-2 text-sm font-medium text-white text-left">Espacio</th>@endif
